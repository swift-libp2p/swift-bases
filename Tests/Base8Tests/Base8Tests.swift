...
final class Base8Tests: XCTestCase {
    ...
    func testEncode() {
        let tests = ["Decentralize everything!!" : "2106254331267164344605543227514510062566312711713506415133463441102=====",
                     "yes mani !" : "362625631006654133464440102=====",
                     "hello world" : "320625543306744035667562330620=="]
        
        for test in tests {
            let encoded = Base8.encode(test.key)
            print("Base8.encode(\(test.key)) => \(encoded)")
    ...
    func testDecode() {
        ...
        for test in tests {
            do {
                ...
                print("Base8.decode(\(test.value)) => \(str ?? "nil")")
